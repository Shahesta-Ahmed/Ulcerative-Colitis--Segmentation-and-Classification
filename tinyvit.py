...
if __name__ == '__main__':
    ...
    pretrained_vit_weights = 'vit_tiny_patch16_224'
    pretrained_vit = timm.create_model(pretrained_vit_weights, pretrained=True).to(device)


    # Change the classifier head
    class_names = ['M0','M1','M2','M3']
    pretrained_vit.head = nn.Linear(in_features=192, out_features=len(class_names)).to(device)
    ...
    final_checkpoint_path = os.path.join('/Users/shahestaahmed/Desktop', 'final_vit_model_tiny.pth')
    torch.save(pretrained_vit.state_dict(), final_checkpoint_path)

    # Save training history
    history_path = '/Users/shahestaahmed/Desktop/history_vit_model_tiny.json'
